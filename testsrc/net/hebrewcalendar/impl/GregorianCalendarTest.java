...
public class GregorianCalendarTest
{
    ...
    public void addDays()
    {
        IDate d1 = new DateImpl(GregorianCalendar.INSTANCE, 2000,1,1);
        IDate d2 = new DateImpl(GregorianCalendar.INSTANCE, 1900,1,1);
        IDate d3 = new DateImpl(GregorianCalendar.INSTANCE,1996,2,22);
        assertEquals(new DateImpl(cal,2001, 1, 2), d1.addDays(367));
        assertEquals(new DateImpl(cal,2000, 12, 30), d1.addDays(364));
        assertEquals(new DateImpl(cal,1900, 12, 31), d2.addDays(364));
        assertEquals(new DateImpl(cal, 1997, 2, 21), d3.addDays(365));
    }

    @Test
    public void subtractDays()
    {
        DateImpl d0 = new DateImpl(cal,1900,2,4);
        DateImpl d1 = new DateImpl(cal,1901,2,4);
    ...
    public void getAbsDay()
    {
        ...
        DateImpl d1 = new DateImpl(cal,2,1,1);
        assertEquals(1373430+365, d1.absDay());

        DateImpl d1996 = new DateImpl(cal, 1996,1,11);
        DateImpl d1997 = new DateImpl(cal,1997,1,11);
        assertEquals(366, d1997.absDay()-d1996.absDay());

        DateImpl d1998 = new DateImpl(cal,1998,1,11);
        assertEquals(365, d1998.absDay()-d1997.absDay());

        DateImpl d1898 = new DateImpl(cal,1898,1,11);
        ...
        for (int y = 1; y < 2100; y+=111) {
            for (int m = 1; m <=12; m++) {
                for (int d =1; d <= cal.monthLength(y,m); d++) {
                    DateImpl nextDate = new DateImpl(cal, y, m, d);
        ...
        DateImpl d20161003 = new DateImpl(cal, 2016,10, 3);
    ...
    {
        IDate d0 = new DateImpl(cal, 2017,7,31);
    ...
    public void getDayOfWeek()
    {
        ...
        DateImpl d401 = new DateImpl(cal, 401,1,1);
        assertEquals((2 + 400 * 365 + 97)%7, d401.getDayOfWeek()%7);

        DateImpl d2001 = new DateImpl(cal, 2001,1,1);
        assertEquals((2+(400 * 365 + 97)*5)%7, d2001.getDayOfWeek()%7);

        DateImpl d20170721 = new DateImpl(cal,2017,7,21);
        assertEquals(6, d20170721.getDayOfWeek());

        DateImpl d20170722 = new DateImpl(cal,2017,7,22);
        assertEquals(7, d20170722.getDayOfWeek());

        DateImpl d20170723 = new DateImpl(cal,2017,7,23);
        assertEquals(1, d20170723.getDayOfWeek());

        DateImpl d20110909 = new DateImpl(cal,2011,9,9);
        assertEquals(6, d20110909.getDayOfWeek());

        DateImpl d20010911 = new DateImpl(cal,2001,9,11);
